...
#define PercentileFun(T, data_reader, cnt_data, alpha)                         \
  {                                                                            \
    ...
    if (pos < 1) {                                                             \
      ...
      if (pos > cnt_data / 2) {                                                \
        ArrayArgs<T>::ArgMaxAtK<data_size_t>(&ref_data, 0, cnt_data, pos - 1); \
        T v1 = ref_data[pos - 1];                                              \
        T v2 = ref_data[pos + ArrayArgs<T>::ArgMax(ref_data.data() + pos,      \
                                                   cnt_data - pos)];           \
        return static_cast<T>(v1 - (v1 - v2) * bias);                          \
      } else {                                                                 \
        ArrayArgs<T>::ArgMaxAtK<data_size_t>(&ref_data, 0, cnt_data, pos);     \
        T v2 = ref_data[pos];                                                  \
...
class RegressionL1loss: public RegressionL2loss {
 public:
  ...
  double RenewTreeOutput(double, std::function<double(const label_t*, data_size_t)> residual_getter,
...
class RegressionMAPELOSS : public RegressionL1loss {
 public:
  ...
  double RenewTreeOutput(
      double,
      std::function<double(const label_t*, data_size_t)> residual_getter,
                         const data_size_t* index_mapper,
                         const data_size_t* bagging_mapper,
                         data_size_t num_data_in_leaf) const override {
    const double alpha = 0.5;
